                        ...
                        if context.state.startTime == nil {
                            if let endTime = context.state.endTime,
                               let endDate = ISO8601DateFormatter().date(
                                   from: endTime.replacingOccurrences(of: ".000", with: "")
                               )
                            {
                                let uiFont = UIFont.systemFont(ofSize: 12, weight: .bold)
            ...
            HStack {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack {
                        ...
                        let _ = print("End time: \(String(describing: context.state))")
                        if context.state.startTime == nil {
                            if let endTime = context.state.endTime,
                               let endDate = ISO8601DateFormatter().date(
                                   from: endTime.replacingOccurrences(of: ".000", with: "")
                               )
                            {
                                ...
                                Text(endDate, style: .timer)
                                  ...
                                  let startDate = ISO8601DateFormatter().date(
                                      from: startTime.replacingOccurrences(of: ".000", with: "")
                                  )
                        {
